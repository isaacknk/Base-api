...
var { playstore, nerding } = require('./apis/scraper.js')
var { PlayStoreSearch } = require('./lib/scraper/pesquisas.js')
var { Estadao } = require('./lib/scraper/noticias.js')
...
bala = await getBuffer(`https://blackzin.sirv.com/Plaq18/20220212_213215.jpg?text.0.text=${textoo}&text.0.position.gravity=northwest&text.0.position.x=43%25&text.0.position.y=18%25&text.0.size=15&text.0.color=000000&text.0.opacity=57&text.0.font.family=Vollkorn&text.0.font.weight=800&text.0.font.style=italic&text.0.background.color=000000&text.0.outline.blur=32&text.0.outline.opacity=46&text.1.text=Sandro bot%3F&text.1.position.gravity=center&text.1.position.x=10%25&text.1.position.y=30%25&text.1.size=20&text.1.color=000000&text.1.opacity=59&text.1.font.family=Playball&text.1.font.weight=700&text.1.outline.opacity=0" width="718" height="1009" alt="" />`)
res.type('jpg')
res.send(bala)
})
...
app.get('/api/nerding', async (req, res, next) => {
    try {
        ...
        if (!key || !Array.isArray(key)) return res.json({ erro: 'Erro no servidor: banco de keys não carregado' });

        const keyIndex = key.findIndex(i => i?.apikey === apikey);
        if (keyIndex === -1) return res.sendFile(path.join(__dirname, "./public/", "limited.html"));

        if (key[keyIndex]?.request <= 0) return res.sendFile(path.join(__dirname, "./public/", "limited.html"));
        ...
        nerding(q)
            ...
            .catch(e => {
                console.error("Erro no nerding:", e);
                res.json({ msg: "Erro no Servidor Interno" });
            });
